...
// import { SafeAreaView, FlatList, ActivityIndicator, Text, StyleSheet, View, Appearance } from 'react-native';
...
//         <ActivityIndicator size="large" color="#0000ffa2" />
...
//       <View style={{backgroundColor:theme.primary, padding:10, borderBottomWidth:1, borderColor:'#ccc'}}> 
//         <Text style={{fontSize:16, textAlign:'center', marginVertical:6, color:'#fff'}}>Approved Template List</Text>
//       </View>
//       <FlatList
//         data={templates}
...
import { SafeAreaView, FlatList, ActivityIndicator, Text, StyleSheet, View, Appearance, TextInput, RefreshControl } from 'react-native';
import TemplateCard from '../Components/TemplateCard';
import { UserContext } from '../Context/UserContext';
import { COLORS } from '../Constants/Colors';
import { API_URL } from '../config';
...
// In-memory cache to store templates after the first fetch
...
const TemplateListScreen = ({ route }) => {
    ...
    // Effect to handle filtering based on searchQuery
    useEffect(() => {
        if (searchQuery) {
            ...
            const filteredData = templates.filter(item => {
                ...
                return (
                    ...
                    (item.Template && item.Template.toLowerCase().includes(lowerCaseQuery)) ||
                    (item.EntityId && item.EntityId.toLowerCase().includes(lowerCaseQuery)) ||
                    (item.TemplateID && item.TemplateID.toLowerCase().includes(lowerCaseQuery)) ||
                    (item.SenderID && item.SenderID.toLowerCase().includes(lowerCaseQuery)) ||
                    (item.TemplateName && item.TemplateName.toLowerCase().includes(lowerCaseQuery)) ||
                    (item.TemplateType && item.TemplateType.toLowerCase().includes(lowerCaseQuery)) ||
                    (item.ImagePath && item.ImagePath.toLowerCase().includes(lowerCaseQuery)) ||
                    ...
                    (item.ServiceType && item.ServiceType.toLowerCase().includes(lowerCaseQuery))
                );
            });
