...
//         ]}
//       >
//         {item.Imagepath && renderAttachment(item)}
//         {item.MessageText && (
//           <Text style={styles.messageText}>{item.MessageText}</Text>
//         )}
//         {item.Attachement_Caption && !item.Imagepath && (
...
const ChatItem = React.memo(
  ({ item, mediaUrls }) => {
    ...
    const isSent = item.Mode === 'Send';
    const mediaUrl = mediaUrls[item.Uid] || item.Imagepath;
  ...
  (prevProps, nextProps) =>
    ...
    prevProps.mediaUrls[prevProps.item.Uid] ===
      nextProps.mediaUrls[nextProps.item.Uid],
);
...
const ChatScreen = ({ route }) => {
  ...
  const fetchMediaUrl = async (filedata, uid) => {
    try {
      const config = {
        method: 'get',
        url: filedata,
        headers: {
          Authentication: `Bearer ${user.WhatsAppBearer}`,
        },
      };
      console.log('user berer token. >>>>>>>', user.WhatsAppBearer);
      console.log('config config config. >>>>>>>', config);
      console.log('filedata filedata filedata. >>>>>>>', filedata);

      const response = await axios.request(config);
      const mediaUrl = response.data?.data?.url;
      if (mediaUrl) {
        setMediaUrls(prev => ({ ...prev, [uid]: mediaUrl }));
      }
    } catch (error) {
      console.error(`Error fetching media URL for ${filedata}:`, error);
    }
  };

  useEffect(() => {
    // Fetch media URLs for all chats with attachments
    chats.forEach(chat => {
      if (chat.Imagepath && !mediaUrls[chat.Uid]) {
        fetchMediaUrl(chat.Imagepath, chat.Uid);
    ...
    closeWebSocket(ws, reconnectTimeoutRef).then(() => {
      ...
      ws.current.onmessage = e => {
        try {
          ...
          if (!receivedUids.current.has(msg.Uid)) {
            if (msg.Imagepath) {
              fetchMediaUrl(msg.Imagepath, msg.Uid);
            }
            if (msg.Mode === 'Send') {
              setChats(prev => {
                let replaced = false;
                const newChats = prev.map(c => {
                  if (
                    ...
                    const updatedMsg = { ...c, ...msg, readstatus: 'sent' };
                    if (c.Uid !== msg.Uid) {
                      receivedself: 'flex-end', Uids.current.delete(c.Uid);
                      receivedUids.current.add(msg.Uid);
                    }
                    return updatedMsg;
                  }
  ...
  const fetchChats = async sinceDate => {
    try {
      const response = await fetch(
        'https://www.loadcrm.com/whatsappmobileapis/api/GetClientChat',
        {
  ...
  const fetchInitialChats = async () => {
    ...
    try {
      ...
      const sinceDate = yesterdayStart.toISOString();

      const newChats = await fetchChats(sinceDate);
      ...
      if (newChats.length > 0) {
        const oldest = new Date(newChats[0].ProcessDate);
        setOldestLoadedDate(oldest);
      } else {
        setOldestLoadedDate(yesterdayStart);
      }
    } catch (error) {
  ...
  const fetchNewerChats = async () => {
    if (chats.length === 0) return;
    const latestDate = new Date(
      chats[chats.length - 1].ProcessDate,
    ).toISOString();
    const newChats = await fetchChats(latestDate);
    if (newChats.length > 0) {
      setChats(prev =>
        [...prev, ...newChats].sort(
          (a, b) => new Date(a.ProcessDate) - new Date(b.ProcessDate),
  ...
  const fetchOlderDayChats = async () => {
    if (isLoadingOlder || !oldestLoadedDate) return;
    setIsLoadingOlder(true);
    try {
      const prevDayStart = new Date(oldestLoadedDate);
      prevDayStart.setDate(prevDayStart.getDate() - 1);
      prevDayStart.setHours(0, 0, 0, 0);
      const prevDayEnd = new Date(prevDayStart);
      prevDayEnd.setHours(23, 59, 59, 999);

      const sinceDate = prevDayStart.toISOString();

      const allFetched = await fetchChats(sinceDate);

      const olderChats = allFetched.filter(c => {
        const chatDate = new Date(c.ProcessDate);
        return chatDate >= prevDayStart && chatDate <= prevDayEnd;
      });
      ...
      if (olderChats.length > 0) {
        ...
        setOldestLoadedDate(prevDayStart);
      } else {
        setOldestLoadedDate(prevDayStart);
        await fetchOlderDayChats();
      }
    } catch (error) {
      console.error('Error fetching older chats:', error);
    } finally {
    ...
    ImagePicker.launchImageLibrary(
      ...
      response => {
        if (response.didCancel) {
          ...
          const uri = response.assets[0].uri;
          console.log('Gallery selected: ', uri);
          // TODO: Implement send image attachment logic
        }
      },
    );
  };
    ...
    ImagePicker.launchImageLibrary(
      ...
      response => {
        if (response.didCancel) {
          ...
          const uri = response.assets[0].uri;
          console.log('Video selected: ', uri);
          // TODO: Implement send video attachment logic
        }
      },
    );
  };
  ...
  const handleChooseDocument = async () => {
    ...
    try {
      ...
      console.log('Document selected: ', res.uri);
      // TODO: Implement send document attachment logic
    } catch (err) {
      if (isCancel(err)) {
        console.log('User cancelled document');
      } else {
  ...
  const handleChooseAudio = async () => {
    ...
    try {
      ...
      console.log('Audio selected: ', res.uri);
      // TODO: Implement send audio attachment logic
    } catch (err) {
      if (isCancel(err)) {
        console.log('User cancelled audio');
      } else {
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={styles.container}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <ChatItem item={item} mediaUrls={mediaUrls} />
            )}
            contentContainerStyle={{ paddingVertical: 10, paddingBottom: 20 }}
            inverted
            showsVerticalScrollIndicator={true}
            ...
            removeClippedSubviews={false}
            onEndReachedThreshold={0.2}
            onEndReached={fetchOlderDayChats}
            ListHeaderComponent={
              isLoadingOlder ? (
                <View style={{ padding: 10, alignItems: 'center' }}>
                  <ActivityIndicator
