...
//   ({ item, index, onPress, onLongPress, isPinned }) => (
//     <TouchableOpacity
//       style={styles.item}
//       onPress={() => onPress(item)}
//       onLongPress={() => onLongPress(item.number)}
//     >
//       <View style={styles.avatar}>
//         <Text style={styles.avatarText}>{index + 1}</Text>
//       </View>
...
//             <Text style={styles.lastDate}>{item.lastDateLabel}</Text>
//             {isPinned && <Feather name="map-pin" size={14} color="#888" />}
//           </View>
//         </View>
//         <Text style={styles.lastMessage} numberOfLines={1}>
//           {item.lastMessage}
...
//       try {
//         const storedPinned = await AsyncStorage.getItem('pinnedOwnChats');
//         if (storedPinned) {
...
//   // Save pinned chats to AsyncStorage
//   useEffect(() => {
//     const savePinned = async () => {
//       try {
//         await AsyncStorage.setItem('pinnedOwnChats', JSON.stringify(pinned));
//       } catch (error) {
...
//   const handleLongPress = useCallback(number => {
//     setPinned(prev => {
//       if (prev.includes(number)) {
//         return prev.filter(n => n !== number); // Unpin
//       } else {
//         if (prev.length >= 10) {
//           return prev; // Max 10 pins limit
//         }
//         return [...prev, number]; // Pin
//       }
//     });
//   }, []);
...
//     const pinnedChats = contactArray
//       .filter(chat => pinned.includes(chat.number))
//       .sort((a, b) => (b.lastDate || 0) - (a.lastDate || 0))
//       .map(chat => ({
//         ...chat,
//         lastDateLabel: getDateLabel(chat.lastDate),
...
//     const unpinnedChats = contactArray
//       .filter(chat => !pinned.includes(chat.number))
//       .sort((a, b) => (b.lastDate || 0) - (a.lastDate || 0))
//       .map(chat => ({
//         ...chat,
...
//         isPinned={pinned.includes(item.number)}
//       />
//     ),
//     [navigateToChat, handleLongPress, pinned],
//   );
